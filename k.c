...
K pe(C *s){s=ws(s);K x,y;C a=*s;I v=qv(a),n=qn(a);if(!v&&!n&&!qa(a))R ke(a);
 if(v){s=ws(++s);if(!*s)R ke(a);R y=pe(s),KE==yt?y:k2(ku(v),y);}
 x=n?pi(&s):ks(*s++);if(KE==xt)R x;s=ws(s),a=*s++;if(!a)R x;v=qv(a);
 if(!v||!*(s=ws(s)))R r0(x),ke(a);R y=pe(s),KE==yt?r0(x),y:k3(kv(v),x,y);}

// print
V p0(K x){-KI==xt?O("%ld",*xi):-KS==xt?O("`%c",*xc):
