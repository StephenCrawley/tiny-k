...
// parse
C*ws(C*s){W(' '==*s)++s;R s;} I qn(C a){R a>='0'&&a<='9';} 
C vt[]=" :+!#,"; I qv(C a){C *s=sc(vt,a);R s?s-vt:0;}
K pi(C**p){C*s=*p;I i=0;W(qn(*s))i=i*10+*s++-'0';R *p=s,ki(i);}
K pe(C *s){s=ws(s);K x,y;C a=*s;I v=qv(a);if(!v&&!qn(a))R ke(a);
 if(v){s=ws(++s);if(!*s)R ke(a);R y=pe(s),KE==yt?y:k2(ku(v),y);}
 x=pi(&s),s=ws(s),a=*s++;if(!a)R x;v=qv(a);if(!v||!*(s=ws(s)))R r0(x),ke(a);
 R y=pe(s),KE==yt?r0(x),y:k3(kv(v),x,y);}

// print
V p0(K x){-KI==xt?O("%ld",*xi):KE==xt?O("'%c",*xc):O(KU==xt?"%c:":"%c",vt[*xi]);}
